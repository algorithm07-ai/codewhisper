...
const Logger_1 = require("../Logger"); // Using the new logger
class ModelManager {
    constructor(maxSize = 3) {
        ...
        if (maxSize <= 0) {
            Logger_1.logger.error('ModelManager: Max cache size must be a positive number.', { maxSize });
            throw new Error("Max cache size must be a positive number.");
        }
        Logger_1.logger.info(`ModelManager initialized with max cache size: ${maxSize}`);
    }
    /**
    ...
    async loadModel(modelId, loadFn) {
        if (this.modelCache.has(modelId)) {
            ...
            Logger_1.logger.debug(`ModelManager: Model ${modelId} loaded from cache.`);
            return this.modelCache.get(modelId);
        }
        Logger_1.logger.info(`ModelManager: Model ${modelId} not in cache. Loading...`);
        const model = await loadFn();
        if (model.modelId !== modelId) {
            Logger_1.logger.error(`ModelManager: Loaded model ID '${model.modelId}' does not match requested ID '${modelId}'.`, { requestedModelId: modelId, loadedModelId: model.modelId });
        ...
        if (this.modelCache.size >= this.maxCacheSize && this.maxCacheSize > 0) {
            ...
            if (lruModelId) {
                ...
                Logger_1.logger.info(`ModelManager: Cache full. Evicted model ${lruModelId}.`);
                if (evictedModel?.dispose) {
                    try {
                        await evictedModel.dispose();
                        Logger_1.logger.debug(`ModelManager: Disposed evicted model ${lruModelId}.`);
                    }
                    catch (e) {
                        Logger_1.logger.error(`ModelManager: Error disposing evicted model ${lruModelId}.`, { error: e });
                    }
                }
            }
        }
        ...
        Logger_1.logger.info(`ModelManager: Model ${modelId} loaded and cached. Cache size: ${this.modelCache.size}`);
        return model;
    }
    /**
     ...
     */
    async clearCache() {
        Logger_1.logger.info('ModelManager: Clearing cache...');
        for (const model of this.modelCache.values()) {
            if (model.dispose) {
                try {
                    await model.dispose();
                    Logger_1.logger.debug(`ModelManager: Disposed model ${model.modelId} during cache clearing.`);
                }
                catch (e) {
                    Logger_1.logger.error(`ModelManager: Error disposing model ${model.modelId} during cache clearing.`, { error: e });
                }
            }
        }
        this.modelCache.clear();
        this.modelAccessOrder = [];
        Logger_1.logger.info('ModelManager: Cache cleared.');
    }
    /**
